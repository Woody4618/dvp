...
async function runDvPDemo() {
  ...
  try {
    ...
    console.log('⚠️  Airdrop failed - make sure you are running a local validator');
    console.log('   Run: solana-test-validator');
    throw error;
  }

  // Verify balances
  const settlementBalance = await connection.getBalance(settlementAgent.publicKey);
  ...
  console.log(`   Settlement Agent: ${settlementBalance / LAMPORTS_PER_SOL} SOL`);
  ...
  console.log('📊 Account Balances:');
  console.log('\n   Issuer:');
  console.log(`   - Bonds: ${issuerBondsAfter.amount.toString()} (was ${issuerBondsBefore.amount.toString()})`);
  console.log(`   - USDC: ${Number(issuerUSDCAfter.amount) / 1e6} (was ${Number(issuerUSDCBefore.amount) / 1e6})`);
  console.log('\n   Investor:');
  console.log(`   - Bonds: ${investorBondsAfter.amount.toString()} (was ${investorBondsBefore.amount.toString()})`);
  console.log(`   - USDC: ${Number(investorUSDCAfter.amount) / 1e6} (was ${Number(investorUSDCBefore.amount) / 1e6})`);
  ...
  const bondTransferCorrect = 
    Number(issuerBondsAfter.amount) === Number(issuerBondsBefore.amount) - BOND_AMOUNT &&
    Number(investorBondsAfter.amount) === Number(investorBondsBefore.amount) + BOND_AMOUNT;
  ...
  const usdcTransferCorrect = 
    Number(issuerUSDCAfter.amount) === Number(issuerUSDCBefore.amount) + (USDC_AMOUNT * 1e6) &&
    Number(investorUSDCAfter.amount) === Number(investorUSDCBefore.amount) - (USDC_AMOUNT * 1e6);
